    ...
    def get_config_data(self) -> None:
        ...
        if args.config:
            with open(args.config, 'r', encoding='utf-8') as f:
                ...
                self.REPORT_SIZE = js.get('REPORT_SIZE') if js.get('REPORT_SIZE') else self.REPORT_SIZE
                self.REPORT_DIR = js.get('REPORT_DIR') if js.get('REPORT_DIR') else self.REPORT_DIR
                self.LOG_DIR = js.get('LOG_DIR') if js.get('LOG_DIR') else self.LOG_DIR
                self.LOG_FILE_PATH = js.get('LOG_FILE_PATH') if js.get('LOG_FILE_PATH') else self.LOG_FILE_PATH
        ...
        for filename in os.listdir(self.LOG_DIR):
            ...
            if file_type == 'gzip' or file_type is None:
                ...
                if match:
                    ...
                    if self.log_file_data.file_date is None or formatted_date > self.log_file_data.file_date:
